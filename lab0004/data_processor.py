...
class DataFrameProcessor:
    ...
    def rename_columns(self) -> None:
        """
        Переименовывает столбцы 'Абсолютный путь' и 'Относительный путь' на английские.
        """
        new_names = {
            "Абсолютный путь": "absolute_path",
            "Относительный путь": "relative_path"
        }
        actual_columns = {old: new for old, new in new_names.items() if old in self.df.columns} # Выбираем только те столбцы, которые реально есть в DataFrame
        if actual_columns:
            self.df.rename(columns=actual_columns, inplace=True) # Переименовываем их

    def _resolve_path(self, row: pd.Series) -> Optional[str]:
        """
        Определяет реальный путь к изображению: сначала абсолютный, потом относительный.
        """
    ...
    def add_brightness_ranges(self, user_bins: List[int]) -> None:
        ...
        for _, row in self.df.iterrows(): # Проходим по каждой строке DataFrame
        ...
        self.current_bin_order = labels + ["Error"]  # Сохраняем порядок меток (включая "Error")

        def get_label(val: int) -> str: # Вложенная функция для определения бина по значению
            if val < 0:
                return "Error"
            if val > 255:
                return "256+"
